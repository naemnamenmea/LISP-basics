;;; #1
;;; Пусть матрицы задаются в виде списка списков. Реализовать функции сложения
;;; и умножения матриц, вычисления определителя матрицы.
...
;;; #2
;;; Написать генератор совершенных чисел. Число называется совершенным, если оно
;;; равно сумме своих делителей.


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;; #3
;;; Близнецами называется пара натуральных чисел, каждое из которых равно сумме
;;; делителей другого числа. Так, например, числа 220 и 284 — близнецы. Написать
;;; генератор первых n пар близнецов.


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;; #4
;;; Заданный числовой список разбить на подсписки из возрастающих подпосле-довательностей
;;; максимальной длины рядом стоящих чисел. Так, например, если исходный список
;;; состоял из чисел [2, 7, 10, 8, 3, 4, 9, 1, 2, 0, 8, 3, 2, 5], то результатом
;;; работы программы должен быть следующий список списков:
...
;;; #5
;;; Написать алгоритм быстрой сортировки (сортировки слиянием) согласно функ-ции
;;; сравнения двух элементов и удалить из полученного списка элементы,
;;; не удовлетворяющие заданному условию. Продемонстрировать на примере спис-ка
;;; студентов, где каждый студент имеет свойства «Фамилия», «Имя», «Груп-па» и «Cредний бал».
;;; В качестве условия–фильтра рассмотреть «Средний бал больше 4», порядок сортировки —
;;; «Группа–Фамилия–Имя–Средний бал».
...
;;; элементами которого являются пары, состоящие из номера вершины и списка вершин,
;;; инцидентных ей. type Graph = (Int, (Int, Int, ...))
;;; Написать функцию, которая выдает длину кратчайшего маршрута между дву-мя
;;; заданными вершинами. Длиной считать количество вершин, встретивших-ся в данном маршруте.
;;; Если маршрута между вершинами не существует, то функция должна выдавать ноль.
...
(defun bfs! (graph chk queue)
   (cond ((null queue) nil)
         (t (let ((n (getall (car queue) graph chk)))
                 (append (mapcar #'(lambda (x) (list (car queue) x)) n) (bfs! graph (append chk n) (append (cdr queue) n)))))))
...
;;; #7
;;; Написать программу для нахождения минимального из чисел, являющихся максимальными
;;; в каждой из строк заданной прямоугольной матрицы. Ис-пользовать применяющие и/или
...
;;; #8
;;; В заданном списке списков найти самый длинный подсписок. Использовать отображающие
;;; и применяющие функционалы.


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;; #9
;;; Выяснить является ли дерево пирамидой — дерево, в котором каждая корневая вершина
;;; меньше любой своей дочерней.
...
;;; #10
;;; Граф с конечным числом вершин, пронумерованных от 1 до N, представлен парой
;;; из количества вершин N и характеристической функции множества его дуг типа
;;; (Int Int) -> Bool, которая выдает T, если аргументы представляют собой номера
;;; вершин, соединенных дугой, и NIL в противном случае.
...
(defun graph (x y)
    (or (find (list x y) graph :test #'equal) (find (list y x) graph :test #'equal))
    )
...
;;; #11
;;; Сформировать список разностей соседних элементов в последовательности квадратов
;;; натуральных чисел. Убедиться, что эта последовательность представля-ет собой список
;;; последовательных нечетных чисел. Использовать применяю-щие и/или отображающие функционалы.


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;; #12
;;; Бесконечная упорядоченная последовательность целых чисел составлена из степеней
;;; двойки и чисел вида 2 ¢ 3n. Написать генератор такой последователь-ности.
...
;;; 6 9 13 18 . . .
;;; 10 14 19 . . .
;;; Построить эту матрицу в виде бесконечного списка бесконечных списков и вычислить
